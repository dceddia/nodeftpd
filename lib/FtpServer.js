...
function FtpServer(host, options) {
  ...
  if(options.implicitSecure) {
    console.log('Creating TLS server');
    self.server = tls.createServer(options.tlsOptions);
    self.server.on('secureConnection', function(socket) {
      self._onConnection(socket);
    });
  } else {
    console.log('Creating insecure server');
...
FtpServer.prototype._onConnection = function(socket) {
  ...
  var conn = new FtpConnection({
    ...
    secure: this.options.implicitSecure || false,
    pbszReceived: false,
  });
